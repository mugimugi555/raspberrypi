...
# wget https://raw.githubusercontent.com/mugimugi555/raspberrypi/main/tv/install_tv.sh && bash install_tv.sh ;

#-----------------------------------------------------------------------------------------------------------------------
# このスクリプトの目的:
#-----------------------------------------------------------------------------------------------------------------------
# PX-S1UDを使用したRaspberry Pi 4での地デジ放送受信環境を構築するためのスクリプト。
# 必要なツール（Docker、Mirakurun、EPGStation）をインストールし、プレイリストファイルを生成するところまで自動化します。
#
#-----------------------------------------------------------------------------------------------------------------------
# 必要な機材:
#-----------------------------------------------------------------------------------------------------------------------
# ・Raspberry Pi 4 (BookWorm 64bit)
# ・PX-S1UD V2.0
# ・SCR3310 カードリーダー
# ・B-CASカード
#
...
# 使用方法:
#-----------------------------------------------------------------------------------------------------------------------
# このスクリプトを実行するだけで、以下を自動で行います:
# 1. 必要なツールのインストール (Docker等)
# 2. PX-S1UD用ドライバのセットアップ
# 3. MirakurunとEPGStationのインストールと初期設定
# 4. チャンネルスキャンの実行
# 5. VLC用のM3Uプレイリストの作成
#
# 実行例:
